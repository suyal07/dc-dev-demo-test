...
# SPECIFIC PRODUCTION resource groups to check (exact matches only)
...
# Safety check: Verify we're only checking production resource groups
Write-Host "🔒 SAFETY CHECK: Verifying target resource groups..." -ForegroundColor Cyan
foreach ($rg in $productionResourceGroups) {
    if ($rg.Name -notmatch "-prod-") {
        ...
        Write-Host "❌ STOPPING SCRIPT - Only production resource groups should be checked!" -ForegroundColor Red
        exit 1
    }
    Write-Host "✅ Verified: $($rg.Name) is a production resource group" -ForegroundColor Green
}

$resourceGroups = $productionResourceGroups

Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "Production Tag Verification Report" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Checking all production resources for complete tag coverage...`n"
...
# Summary Report
Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "Production Test Summary Report" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
if ($allIssues.Count -gt 0) {
    ...
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host "To fix the missing or incorrect tags, run the Apply-Tags.ps1 script:"
    Write-Host "  ./scripts/Apply-Tags.ps1" -ForegroundColor Green
    
    Write-Host "`n❌ PRODUCTION TEST FAILED: Tag compliance issues found!" -ForegroundColor Red
    exit 1
}
else {
    Write-Host "`n✅ All production resources have the complete set of common tags!" -ForegroundColor Green
    Write-Host "✅ PRODUCTION TEST PASSED: All resources properly tagged!" -ForegroundColor Green
}
...
Write-Host "`n========================================" -ForegroundColor Cyan
Write-Host "Production Tag Compliance: $compliancePercentage%" -ForegroundColor $(if ($compliancePercentage -eq 100) { "Green" } else { "Yellow" })
Write-Host "========================================`n" -ForegroundColor Cyan
